...
void rt_schedule_from_ISR_exit(struct rt_thread *from_thread)
{
    rt_base_t level;
    struct rt_thread *to_thread;
...
void rt_schedule_from_ISR_unused(struct rt_thread *to_thread)
{
    rt_base_t level;
    struct rt_thread *from_thread;
    ...
    if (rt_scheduler_lock_nest == 0)
    {
        ...
        if (rt_thread_ready_priority_group != 0)
        {
            ...
            if (to_thread != rt_current_thread)
            {
                ...
                rt_hw_context_switch_interrupt((rt_ubase_t)&to_thread->sp,
                        (rt_ubase_t)&from_thread->sp);
